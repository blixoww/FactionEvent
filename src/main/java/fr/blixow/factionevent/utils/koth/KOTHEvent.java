...
public class KOTHEvent {
  ...
  public KOTHEvent(KOTH koth) {
    ...
    try {
      this.duration = config.contains("koth.max_duration") ? config.getInt("koth.max_duration") : 1800;
    } catch (Exception ignored) {
    }
    try {
      this.win_time = config.contains("koth.win_time") ? config.getInt("koth.win_time") : 300;
    } catch (Exception ignored) {
    }
  }
    ...
    if (playersInKOTH.containsKey(player)) {
      if (isFirstPlayer(player)) {
        if (playersInKOTH.size() == 1) {
          String str = "";
          try {
            str = new StrManager(msg.getString("koth.no_more_king")).reKoth(koth.getName()).toString();
          } catch (Exception exception) {
            exception.printStackTrace();
          }
          Bukkit.broadcastMessage(prefix + str);
  ...
  public LinkedHashMap<Player, Long> getPlayersInKOTH() {
    return playersInKOTH;
  }
  ...
  public boolean checkTimer() {
    ...
    if (playersInKOTH.isEmpty()) {
      ...
      long joined = getFirstPlayerJoined();
      if (!KOTHManager.isInKOTH(player, koth)) {
        removePlayer(player);
        return false;
      }
  ...
  public void grantVictory(Player player) {
    ...
    if (!faction.isWilderness()) {
      int points = 10;
      try {
        if (config.contains("koth.win_points")) {
          points = config.getInt("koth.win_points");
          if (points < 1) {
            points = 1;
          }
        }
