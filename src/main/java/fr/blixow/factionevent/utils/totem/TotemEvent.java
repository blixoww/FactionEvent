...
public class TotemEvent {
  ...
  public TotemEvent(Totem totem) {
    ...
    try {
      if (config.contains("totem.max_duration")) {
        duration = config.getInt("totem.max_duration");
      }
  ...
  private void grantVictory(Player player) {
    ...
    if (faction != null && !faction.isWilderness() && !faction.isSafeZone() && !faction.isWarZone()) {
      int points = 10;
      try {
        if (config.contains("totem.win_points")) {
          points = config.getInt("totem.win_points");
          if (points < 1) {
            points = 1;
          }
        }
  ...
  public void copyBlocks() {
    ...
    if (this.totem.getBlocks() != null) {
      this.blocks.putAll(totem.getBlocks());
    }
  }


  public boolean checkTimer() {
    if (!FactionEvent.getInstance().getEventOn().getTotemEvent().equals(this)) {
      return true;
    }
    long now = new Date().getTime();
