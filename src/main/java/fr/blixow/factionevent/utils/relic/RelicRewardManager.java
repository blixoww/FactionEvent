...
/**
 * Stockage centralisé des récompenses de la Course à la Relique en attente de réclamation
 * (/relicreward). Mêmes conventions que {@link fr.blixow.factionevent.utils.lms.LMSRewardManager}.
 */
public class RelicRewardManager {
    ...
    public static void loadRewards() {
        ...
        try {
            FileConfiguration fc = FileManager.getRelicRewardsDataFC();
            if (fc == null) return;
            ...
            Bukkit.getConsoleSender().sendMessage("[Relique] " + pendingRewards.size()
    ...
    public static void saveRewards() {
        try {
            File file = FileManager.getDataFile("relicRewards.yml");
            FileConfiguration fc = FileManager.getRelicRewardsDataFC();
            if (fc == null) return;
            fc.set("rewards", null);
            ...
            fc.save(file);
            FactionEvent.getInstance().setRelicRewardsFileConfiguration(fc);
        } catch (Exception e) { e.printStackTrace(); }
    }
