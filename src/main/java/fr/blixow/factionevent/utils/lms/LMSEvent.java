...
public class LMSEvent {
    ...
    public void handlePlayerQuit(Player player) {
        if (!participants.containsKey(player)) return;
        participants.remove(player);
        Bukkit.broadcastMessage(prefix + new StrManager(msg.getString("lms.eliminated", "§c{player} §7a quitté le LMS §e{lms}§7 et est éliminé."))
                .rePlayer(player).reLMS(lms.getName()).toString());
        ...
        if (participants.size() == 1) {
            ...
            if (eventActive) updateScoreboard();
        }
    }
    ...
    public void grantVictory(Player player) {
        ...
        if (!faction.isWilderness()) {
            int points = 10;
            try {
                if (config.contains("lms.win_points")) {
                    points = Math.max(1, config.getInt("lms.win_points"));
                }
            } catch (Exception ignored) {}
            RankingManager.addLMSWins(faction);
            RankingManager.addPoints(faction, points);
            if (player.isOnline()) {
                FactionMessageTitle.sendFactionTitle(faction, 20, 40, 20, "§aLMS remporté", "+" + points + " points au classement");
            }
        }
