...
public class DTCEvent {
    ...
    public DTCEvent(DTC dtc){
        ...
        this.max_vie = 50000; // todo récupérer depuis fichier config
        this.vie = this.max_vie; // todo récupérer depuis fichier config
        ...
        try { if(cf.contains("dtc.check_hit")){ this.check_hit = cf.getInt("dtc.check_hit"); } } catch (Exception ignored){}
        if(check_hit < 1){ check_hit = 1; }
    ...
    public void hit(Player player, double damage){
        ...
        if(!faction.isWilderness() && !faction.isSafeZone() && !faction.isWarZone()){
            ...
            if(damageMap.containsKey(faction)){ current_faction_damage = damageMap.get(faction); }
        ...
        for(Faction faction1 : damageMap.keySet()){ FactionMessageTitle.sendFactionActionBar(faction1, vie_restante_actionbar); }
        if(isDead()){ vie = 0; grantVictory(); }
        else {
            nombre_coup++;

            if(nombre_coup % check_hit == 0){ updateScoreboard(); } }
    }
    ...
    public void updateScoreboard(){
        if(vie > 0){
            ...
            if(highest_faction != null && !highest_faction.isWilderness() && !highest_faction.isSafeZone() && !highest_faction.isWarZone()){ factionName = highest_faction.getTag(); }
    ...
    private Faction getHighestDamageFaction(){
        ...
        for(Faction faction : damageMap.keySet()){
            ...
            if(faction_damage > max_damage){ max_faction = faction; max_damage = faction_damage; }
        }
        return max_faction;
    }
