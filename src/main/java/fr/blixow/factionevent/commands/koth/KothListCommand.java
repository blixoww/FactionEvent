...
public class KothListCommand implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    ...
    try {
      title = msg.getString("kothlist.title");
    ...
    if (FactionEvent.getInstance().getListKOTH().isEmpty()) {
      try {
        empty = msg.getString("kothlist.empty");
      } catch (Exception exception) {
        exception.printStackTrace();
      }
    ...
    for (KOTH koth : FactionEvent.getInstance().getListKOTH()) {
      if (KOTHManager.isKOTHStarted(koth)) {
        ...
        try {
          message = new StrManager(msg.getString("kothlist.item-started")).reKoth(koth.getName()).toString();
        } catch (Exception exception) {
          exception.printStackTrace();
        }
        ...
        try {
          message = new StrManager(msg.getString("kothlist.item")).reKoth(koth.getName()).toString();
        } catch (Exception exception) {
          exception.printStackTrace();
        }
        sender.sendMessage(message);
