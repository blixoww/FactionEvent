...
public class DTCListCommand implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    ...
    try {
      title = msg.getString("dtclist.title");
    ...
    if (FactionEvent.getInstance().getListDTC().isEmpty()) {
      try {
        empty = msg.getString("dtclist.empty");
      } catch (Exception exception) {
        exception.printStackTrace();
      }
    ...
    for (DTC dtc : FactionEvent.getInstance().getListDTC()) {
      if (DTCManager.isDTCStarted()) {
        ...
        try {
          message = new StrManager(msg.getString("dtclist.item-started")).reDTC(dtc.getName()).toString();
        } catch (Exception exception) {
          exception.printStackTrace();
        }
        ...
        try {
          message = new StrManager(msg.getString("dtclist.item")).reDTC(dtc.getName()).toString();
        } catch (Exception exception) {
          exception.printStackTrace();
        }
        sender.sendMessage(message);
