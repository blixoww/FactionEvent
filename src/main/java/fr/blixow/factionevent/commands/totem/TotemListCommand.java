...
public class TotemListCommand implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    ...
    try {
      title = msg.getString("totemlist.title");
    ...
    if (FactionEvent.getInstance().getListTotem().isEmpty()) {
      try {
        empty = msg.getString("totemlist.empty");
      } catch (Exception exception) {
        exception.printStackTrace();
      }
    ...
    for (Totem totem : FactionEvent.getInstance().getListTotem()) {
      if (TotemManager.isTotemStarted(totem)) {
        ...
        try {
          message = new StrManager(msg.getString("totemlist.item-started")).reTotem(totem.getName()).toString();
        } catch (Exception exception) {
          exception.printStackTrace();
        }
        ...
        try {
          message = new StrManager(msg.getString("totemlist.item")).reTotem(totem.getName()).toString();
        } catch (Exception exception) {
          exception.printStackTrace();
        }
        sender.sendMessage(message);
