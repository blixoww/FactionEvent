...
public class MeteoriteListCommand implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    ...
    try {
      title = msg.getString("meteoritelist.title");
    ...
    if (FactionEvent.getInstance().getListMeteorite().isEmpty()) {
      try {
        empty = msg.getString("meteoritelist.empty");
      } catch (Exception exception) {
        exception.printStackTrace();
      }
    ...
    for (Meteorite meteorite : FactionEvent.getInstance().getListMeteorite()) {
      if (MeteoriteManager.isMeteoriteStarted(meteorite)) {
        ...
        try {
          message = new StrManager(msg.getString("meteoritelist.item-started")).reMeteorite(meteorite.getName()).toString();
        } catch (Exception exception) {
          exception.printStackTrace();
        }
        ...
        try {
          message = new StrManager(msg.getString("meteoritelist.item")).reMeteorite(meteorite.getName()).toString();
        } catch (Exception exception) {
          exception.printStackTrace();
        }
        sender.sendMessage(message);
