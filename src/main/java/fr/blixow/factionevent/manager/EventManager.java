...
public class EventManager {
  ...
  public void switchScoreboard(Player player) {
    ...
    if (meteoriteEvent != null) {
      meteoriteEvent.getScoreBoardAPI().getObjective().unregister();
      meteoriteEvent.updateScoreboard();
    }
  ...
  public void switchTitle(Player player) {
    ...
    if (this.title) {
      player.sendMessage("§7Les titres d'événement ont été §aactivé§7!");
    } else {
      player.sendMessage("§7Les titres d'événement ont été §cdésactivé§7!");
    }
  }
  ...
  public void switchActionbar(Player player) {
    ...
    if (this.actionbar) {
      player.sendMessage("§7Les message dans la barre d'action on été §aactivé§7!");
    } else {
      player.sendMessage("\"§7Les message dans la barre d'action on été §cdésactivé§7!");
    }
  }
