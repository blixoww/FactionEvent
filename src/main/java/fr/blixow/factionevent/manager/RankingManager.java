    ...
    public static void logsMessage(String message) {
        try {
            ...
            if (logs.contains(path)) {
                stringList = logs.getStringList(path);
            }
    ...
    public static String getFactionsInformations(FileConfiguration fc, String id) {
        try {
            if (fc.contains(id)) {
                ...
                if (fc.contains(id + ".koth")) {
                    koth = String.valueOf(fc.getInt(id + ".koth"));
                }
                if (fc.contains(id + ".totem")) {
                    totem = String.valueOf(fc.getInt(id + ".totem"));
                }
        ...
        try {
            while (!unsortedMap.isEmpty()) {
                ...
                if (current_max == 0) {
                    ...
                    sortedMap.put(current_faction, current_max);
                    factionsAlreadySet.add(current_faction);
                }
                for (Faction faction : factionsAlreadySet) {
                    unsortedMap.remove(faction);
                }
            }
        } catch (Exception exception) {
            exception.printStackTrace();
